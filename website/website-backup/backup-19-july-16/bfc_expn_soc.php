	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
		...
		<tr>
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  <td class="backheads" align="center">Share of Budget for Children in Social Sector Expenditure </td>
			  </tr>
			<tr>
			  <td align="right">&nbsp; </td>
			</tr>
...
// For Excel File Generation
			...
			$_SESSION['report_header']=array("Share of Budget for Children in Social Sector Expenditure"," "); 
				$i=0;
				$_SESSION['report_values'][$i][0]=Year."  ";
				$_SESSION['report_values'][$i][1]=Share." ".of." ".Budget." "."for"." ".Children." ".in." ".Social." ".Sector." ".Expenditure." ";
				$i=1;
			while($fetch_year=mysql_fetch_array($result_year)){


			$select_social_budget="select budget_ae_grand_total, budget_year from haq_budget_report_union where budget_type='social_sec' and budget_status='active' and social_sector_type='".$_REQUEST['level']."' and budget_year='".$fetch_year['year_name']."'";
			$result_social_budget=mysql_query($select_social_budget);
			$fetch_social_budget=mysql_fetch_array($result_social_budget);

			$select_ae="select sum(ae_grand_total) as ae_total, expend_year from haq_head_expend_union where expend_type='".$_REQUEST['level']."' and expend_status='active' and expend_year='".$fetch_year['year_name']."' group by expend_year";
...
/*			$select_soc_all="select sum(budget_ae_grand_total) as soc_total from haq_budget_report_union where budget_type='social_sec' and budget_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and social_sector_type='".$_REQUEST['level']."' and budget_status='active'";
			$result_soc_all=mysql_query($select_soc_all);
			$fetch_soc_all=mysql_fetch_array($result_soc_all);

			$select_bfc_all="select sum(ae_grand_total) as ae_total from haq_head_expend_union where expend_type='".$_REQUEST['level']."' and expend_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and expend_status='active'";
...
////////////////// A V E R A G E /////////////////////
				...
				<tr>
				  <td height="30" colspan="7" align="center">&nbsp;<a href="export_report.php?fn=UB_share_BFC_SS_exp">Download in Excel</a></td>
				  </tr>
		  </table></td>
			</tr>
			<tr>
