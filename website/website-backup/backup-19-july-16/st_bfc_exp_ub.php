		...
		<td colspan="2" class="text12"><table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
			...
			<tr>
				<td align="left" valign="top" ><form name="form1" method="post" action=""><table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
					...
					<tr>
						<td class="backheads" align="center">Sectoral Expenditure within Budget for Children, as percentage of the State Budget </td>
					</tr>
					<tr>
						<td align="right">&nbsp; </td>
					</tr>
						...
						<tr>
						  ...
						  <td width="38%" align="left">&nbsp;To : &nbsp;&nbsp;
							...
							<tr>
								...
								<td  align="left"><select name="sector_id"><option value="" selected>Select</option>
									<?php
							...
							<tr>
								<td height="25" colspan="5" class="txt">&nbsp;</td>
							</tr>
<?php
...
<?php } ?>							<tr>
								<td height="25" class="txt" colspan="2"><strong>Year </strong></td>
									<?php
								...
								<td height="25" colspan="2" align="center" class="txt"><strong><?php if($fetch_query['sector_name'] !=''){ echo $fetch_query['sector_name'] ;} else { echo 'Sector' ; } ?></strong></td>
							</tr>
								<?php
									if (isset($_REQUEST['submit'])){
						...
						?>
										<tr>
										  <td height="25" align="center" colspan="5" class="error"><?php echo $error ; ?></td>
										  </tr>
						<?php
									}else {
						...
						// For Excel File Generation
									...
									$_SESSION['report_header']=array("Sectoral Expenditure within Budget for Children, as percentage of the State Budget", " "); 
									$i=0;
									...
									$select_sector_report="select sum(a.ae_grand_total) as ae_total, a.expend_year as year from haq_head_expend_state a LEFT JOIN haq_st_major_head b ON a.major_head_id=b.major_head_id where b.major_head_status='active' and b.sector_id='".$fetch_query['sector_id']."' and a.expend_type='".$_REQUEST['level']."' and a.state_id='".$_REQUEST['state_id']."' and a.expend_year='".$fetch_year['year_name']."' group by a.expend_year";
									$result_sector_report=mysql_query($select_sector_report);
									$fetch_sector_report=mysql_fetch_array($result_sector_report);
									$ae_cro=$fetch_sector_report['ae_total']/10000;
									$select_union_budget="select budget_ae_grand_total, budget_year from haq_budget_report_union where budget_type='".$_REQUEST['level']."' and state_id='".$_REQUEST['state_id']."' and budget_status='active' and social_sector_type='na' and budget_year='".$fetch_year['year_name']."'";
									...
									if ($fetch_union_budget['budget_ae_grand_total'] != 0)
									{
									$ae_bfc= $ae_cro/$fetch_union_budget['budget_ae_grand_total']*100;
									} else {
									$ae_bfc = "N/A" ;
									}
							...
							<tr>
								...
								<td colspan="3" align="center" class="txt"><?php echo round($ae_bfc, 3)."%" ; ?></td>
							</tr>
								<?php
									$i=$i+1;
									}	}
///////AVERAGE CALCULATION STARTS////////////////
							...
							<!--tr>
								...
								<?php
/*
								$select_sector_avg="select sum(a.ae_grand_total) as ae_total from haq_head_expend_state a LEFT JOIN haq_st_major_head b ON a.major_head_id=b.major_head_id where b.major_head_status='active' and a.expend_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and a.state_id='".$_REQUEST['state_id']."' and b.sector_id='".$fetch_query['sector_id']."' and a.expend_type='".$_REQUEST['level']."' and expend_status='active'";
								$result_sector_avg=mysql_query($select_sector_avg);
								$fetch_sector_avg=mysql_fetch_array($result_sector_avg);
								$ae_avg=$fetch_sector_avg['ae_total']/10000;
								
								$select_union_avg="select sum(budget_ae_grand_total) as ae_uni_total from haq_budget_report_union where budget_type='".$_REQUEST['level']."' and budget_status='active' and budget_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and state_id='".$_REQUEST['state_id']."' and social_sector_type='na'";
								$result_union_avg=mysql_query($select_union_avg);
								$fetch_union_avg=mysql_fetch_array($result_union_avg);
								
								if($fetch_union_avg['ae_uni_total'] !=0){
								$ae_bfc_avg= $ae_avg/$fetch_union_avg['ae_uni_total']*100;
									$_SESSION['report_values'][$i][0]=Average."  ";
									$_SESSION['report_values'][$i][1]=round($ae_bfc_avg, 4)."% ";
*/								?>
								<!--td align="center" class="txt" colspan="2" ><?php // echo round($ae_bfc_avg, 4)."%" ; ?></td-->
								<?php
//								}else {
								?>
								<!--td align="center" class="txt" colspan="2" ></td-->
								<?php
	//								}
...
///////AVERAGE CALCULATION ENDS////////////////
							...
							<?php } }?>
							<tr>
								<td height="30" colspan="5" align="center">&nbsp;<a href="export_report.php?fn=ST_sec_exp_bfc_ST">Download in Excel</a></td>
							</tr>
						</table></td>
					</tr>
					<tr>
