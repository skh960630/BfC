...
// For Excel File Generation
			...
			$_SESSION['report_header']=array("Share of Social Sector Allocation in State Budget"," "); 
				$i=0;
				$_SESSION['report_values'][$i][0]=State." >> ".$fetch_state['state_name'];
				$_SESSION['report_values'][$i][1]=" ";
				$i=1;
				$_SESSION['report_values'][$i][0]=Year."  ";
				$_SESSION['report_values'][$i][1]=Share." ".of." ".Social." ".Sector." ".Allocation." ".in." ".State." ".Budget." ";
				$i=2;
			while($fetch_year=mysql_fetch_array($result_year)){


			$select_union_budget="select budget_be_grand_total, budget_year from haq_budget_report_union where budget_type='".$_REQUEST['level']."' and social_sector_type='na' and budget_status='active' and state_id='".$_REQUEST['state_id']."' and budget_year='".$fetch_year['year_name']."'";
			$result_union_budget=mysql_query($select_union_budget);
			$fetch_union_budget=mysql_fetch_array($result_union_budget);

			$select_social_budget="select budget_be_grand_total, budget_year from haq_budget_report_union where budget_type='social_sec' and social_sector_type='".$_REQUEST['level']."' and state_id='".$_REQUEST['state_id']."' and budget_year='".$fetch_year['year_name']."'";
...
///////AVERAGE CALCULATION ENDS////////////////
				...
				<tr>
				  <td height="30" colspan="7" align="center">&nbsp;<a href="export_report.php?fn=ST_allocation_sectorwise">Download in Excel</a></td>
				  </tr>
		  </table></td>
			</tr>
			<tr>
