		...
		<td colspan="2" class="text12"><table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
			...
			<tr>
				<td align="left" valign="top" ><form name="form1" method="post" action=""><table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
					...
					<tr>
						<td class="backheads" align="center">Sectoral Allocation within Budget for Children, as percentage of the State Budget </td>
					</tr>
					<tr>
						<td align="right">&nbsp; </td>
					</tr>
...
<?php } ?>							<tr>
								...
								<td height="25" colspan="3" align="center" class="txt"><strong><?php if($fetch_query['sector_name'] !=''){ echo $fetch_query['sector_name'] ;} else { echo 'Sector' ; } ?></strong></td>
							</tr>
								<?php
									if (isset($_REQUEST['submit'])){
						...
						// For Excel File Generation
									...
									$_SESSION['report_header']=array("Sectoral Allocation within Budget for Children, as percentage of the State Budget", " "); 
									$i=0;
									...
									$select_sector_report="select sum(a.be_grand_total) as be_total, a.expend_year as year from haq_head_expend_state a LEFT JOIN haq_st_major_head b ON a.major_head_id=b.major_head_id where b.major_head_status='active' and b.sector_id='".$fetch_query['sector_id']."' and a.state_id='".$_REQUEST['state_id']."' and a.expend_type='".$_REQUEST['level']."' and a.expend_year='".$fetch_year['year_name']."' group by a.expend_year";
									...
									$select_union_budget="select budget_be_grand_total, budget_year from haq_budget_report_union where budget_type='".$_REQUEST['level']."' and budget_status='active' and state_id='".$_REQUEST['state_id']."' and social_sector_type='na' and budget_year='".$fetch_year['year_name']."'";
							...
							<tr>
								...
								<td colspan="2" align="center" class="txt"><?php echo round($be_bfc, 3)."%" ; ?></td>
							</tr>
								<?php
									$i=$i+1;
									}	}
							...
							<tr>
								...
								$select_union_avg="select sum(budget_be_grand_total) as be_gnrd_total from haq_budget_report_union where budget_type='".$_REQUEST['level']."' and budget_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and state_id='".$_REQUEST['state_id']."' and budget_status='active' and social_sector_type='na'";
								$result_union_avg=mysql_query($select_union_avg);
								$fetch_union_avg=mysql_fetch_array($result_union_avg);
								
								if($fetch_union_avg['be_gnrd_total'] !=0){
								$be_bfc_avg= $be_avg/$fetch_union_avg['be_gnrd_total']*100;
									$_SESSION['report_values'][$i][0]=Average."  ";
									$_SESSION['report_values'][$i][1]=round($be_bfc_avg, 3)."% ";
								?>
								<td align="center" class="txt" colspan="2"><?php echo round($be_bfc_avg, 3)."%" ; ?></td>
								<?php
								}else{
								?>
							...
							<?php  } }?>
							<tr>
								<td height="30" colspan="5" align="center">&nbsp;<a href="export_report.php?fn=ST_sec_all_bfc_ST">Download in Excel</a></td>
							</tr>
						</table></td>
					</tr>
					<tr>
