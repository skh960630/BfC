<?php
	...
	if (isset($_REQUEST['submit'])){ 
		...
		$s_query=mysql_query("select * from haq_head_expend_union where expend_year='".$_REQUEST['bud_year']."' and expend_status='active' and major_head_id='".$fetch_minor_head['major_head_id']."' and sub_major_head_id='".$fetch_minor_head['sub_major_head_id']."' and minor_head_id='".$fetch_minor_head['minor_head_id']."'");
		...
		major_head_id, sub_major_head_id, minor_head_id, program_id, sector_id) 
		VALUES(
		...
		'".$fetch_minor_head['major_head_id']."', '".$fetch_minor_head['sub_major_head_id']."', '".$fetch_minor_head['minor_head_id']."', '".$_REQUEST['program_name']."', '".$fetch_minor_head['sector_id']."')";
		$result_budget=mysql_query($insert_budget);
		$update_program=mysql_query("update haq_major_head set program_id='".$_REQUEST['program_name']."' where major_head_id='".$fetch_minor_head['major_head_id']."'");
			if($result_budget){
			echo "<meta http-equiv='Refresh' content='0; url=minor_head.php?level=union'>";
			exit;
			}else{ 
	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
		...
		<tr>
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  <td width="25%">&nbsp;</td>
			  <td width="34%" align="center" class="backheads">Minor Head Entry </td>
			...
			<tr>
			  ...
			  <td colspan="3" align="left"><select name="program_name">
				...
				<?
					$sel_prog="select * from haq_program where program_status='active' and sector_id='".$fetch_minor_head['sector_id']."' order by program_id";
