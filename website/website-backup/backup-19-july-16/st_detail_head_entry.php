<?php
	...
	include ('conn.php');
		...
		$select_detail_head=mysql_query("select * from haq_st_detail_head where detail_head_id='".$_REQUEST['detail_headid']."' $sub_query");
	...
	if (isset($_REQUEST['submit'])){ 
		...
		major_head_id, sub_major_head_id, minor_head_id, gen_head_id, sub_head_id, detail_head_id, program_id, sector_id, state_id) 
		VALUES(
		...
		'".$fetch_detail_head['major_head_id']."', '".$fetch_detail_head['sub_major_head_id']."', '".$fetch_detail_head['minor_head_id']."', '".$fetch_detail_head['gen_head_id']."', '".$fetch_detail_head['sub_head_id']."', '".$fetch_detail_head['detail_head_id']."', '".$_REQUEST['program_name']."', '".$fetch_detail_head['sector_id']."', '".$fetch_detail_head['state_id']."')";
		$result_budget=mysql_query($insert_budget);
		$update_program=mysql_query("update haq_st_major_head set program_id='".$_REQUEST['program_name']."' where major_head_id='".$fetch_detail_head['major_head_id']."'");
			if($result_budget){
			echo "<meta http-equiv='Refresh' content='0; url=st_detail_head.php?level=state'>";
			exit;
			}else{ 
		...
		<tr>
		  ...
		  <table width="56%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  ...
			  <td width="38%" align="right"><a href="vi_detail_head_entry.php?detail_headid=<?php echo $_REQUEST['detail_headid'] ; ?>" class="back_menu">VI Schedule Entry</a>&nbsp;&nbsp;<a href="st_detail_head.php" class="back_menu">View Detail Head</a>&nbsp;&nbsp;</td>
			</tr>
			<tr>
			...
			<tr>
			  ...
			  <td colspan="3" align="left"><select name="program_name">
				...
				<?
					$sel_prog="select * from haq_st_program where program_status='active' and sector_id='".$fetch_detail_head['sector_id']."' and program_state_id='".$fetch_detail_head['state_id']."' order by program_id";
