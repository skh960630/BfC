<?php
	...
	$s_query=mysql_query("select * from haq_st_minor_head where 1=1 $sub_query order by minor_head_id desc, state_id, sector_id, minor_head_name");
	if(isset($_REQUEST['action'])){
		if($_REQUEST['action']=="delete"){
			$del_query="delete from haq_st_minor_head where minor_head_id=".$_REQUEST['delcode'];
			$del_result=mysql_query($del_query);		
			$del_gen="delete from haq_st_gen_head where minor_head_id=".$_REQUEST['delcode'];
			$del_result3=mysql_query($del_gen);
			$del_sub="delete from haq_st_sub_head where minor_head_id=".$_REQUEST['delcode'];
			$del_result4=mysql_query($del_sub);
			$del_detail="delete from haq_st_detail_head where minor_head_id=".$_REQUEST['delcode'];
			$del_result5=mysql_query($del_detail);
			$del_object="delete from haq_st_object_head where minor_head_id=".$_REQUEST['delcode'];
			$del_result6=mysql_query($del_object);
			$del_other="delete from haq_st_other_head where minor_head_id=".$_REQUEST['delcode'];
			$del_result7=mysql_query($del_other);
			$del_data="delete from haq_head_expend_state where minor_head_id=".$_REQUEST['delcode'];
...
///////////////////for Update status ///////////////////////////
		if($_GET['action']=="status"){
			...
			$upd_query="update haq_st_minor_head set minor_head_status='".$chstatus."' where  minor_head_id=".$_REQUEST['statusid'];
			mysql_query($upd_query);
			$upd_query3="update haq_st_gen_head set gen_head_status='".$chstatus."' where minor_head_id=".$_REQUEST['statusid'];
			mysql_query($upd_query3);
			$upd_query4="update haq_st_sub_head set sub_head_status='".$chstatus."' where minor_head_id=".$_REQUEST['statusid'];
			mysql_query($upd_query4);
			$upd_query5="update haq_st_detail_head set detail_head_status='".$chstatus."' where minor_head_id=".$_REQUEST['statusid'];
			mysql_query($upd_query5);
			$upd_query6="update haq_st_object_head set object_head_status='".$chstatus."' where minor_head_id=".$_REQUEST['statusid'];
			mysql_query($upd_query6);
			$upd_query7="update haq_st_other_head set other_head_status='".$chstatus."' where minor_head_id=".$_REQUEST['statusid'];
	  ...
	  <td colspan="2" class="text12"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
		<tr>
		  <td width="50%" align="left" valign="top" bgcolor="#FFFFFF" class="justify"><table width="100%"  border="0" cellpadding="5" cellspacing="1" class="border1">
			...
			<tr bgcolor="#BCCAD8">
			  <td width="13%" ><div align="left"><strong>Name</strong></div></td>
			  <td width="10%"><div align="left"><strong>Code</strong></div></td>
			  <td width="13%"><div align="left"><strong>State</strong></div></td>
			  <td width="15%"><div align="left"><strong>Sector</strong></div></td>
			  <td width="17%" ><div align="left"><strong>Sub Major Head</strong></div></td>
			...
			<tr bgcolor="#F1F4F7">
			  ...
			  <td><div align="left">&nbsp;&nbsp;
			    <?php 
					$sec_query=mysql_query("select sub_major_head_name from haq_st_sub_major_head where sub_major_head_id='".$r_query['sub_major_head_id']."' and sub_major_head_status='active'");
					$sec_result=mysql_fetch_array($sec_query);
					echo $sec_result['sub_major_head_name'] ; ?>
			    </div></td>
			  <td width="7%" align="center" bgcolor="#F1F4F7"><a href="st_edit_minor_head.php?minorheadid=<?php echo $r_query['minor_head_id'] ; ?>"><img src="images/edit.gif" alt="Edit" width="16" height="16" border="0"></a></td>
			  <td width="7%" align="center" bgcolor="#F1F4F7"><a href="st_minor_head.php?action=delete&delcode=<?php echo $r_query['minor_head_id'] ; ?>"><img src="images/delete.gif" alt="Delete" width="16" height="16" border="0"></a></td>
			  <td width="6%" align="center" bgcolor="#F1F4F7"><a href="st_minor_head.php?action=status&statuscode=<?php echo $r_query['minor_head_status'] ; ?>&statusid=<?php echo $r_query['minor_head_id'] ; ?>"><?php echo $r_query['minor_head_status']?></a></td>
			  <td width="12%" align="center" bgcolor="#F1F4F7"><a href="st_add_gen_head.php?minor_headid=<?php echo $r_query['minor_head_id'] ; ?>">Add General Head</a></td>
			</tr> 
			<?php } ?>
		  </table></td>
		</tr>
