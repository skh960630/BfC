<?php 
	...
	if(isset($_REQUEST['submit'])){
			$select_sub_major_head=mysql_query("select * from haq_sub_major_head where sub_major_head_id='".$_REQUEST['submajor_headid']."'");
			$fetch_sub_major_head=mysql_fetch_array($select_sub_major_head);

		$s_query=mysql_query("select * from haq_minor_head where sub_major_head_id='".$fetch_sub_major_head['sub_major_head_id']."' and major_head_id='".$fetch_sub_major_head['major_head_id']."' and minor_head_name='".$_REQUEST['minor_head_name']."' and minor_head_code='".$_REQUEST['minor_head_code']."'");
		$f_query=mysql_num_rows($s_query);
		if($f_query !=0){
			$error= "Sorry, The Minor Head " . $_REQUEST['minor_head_name'] . " or Code " .$_REQUEST['minor_head_code'] . " is already in use.";
		} else {

			$i_query="INSERT INTO haq_minor_head (minor_head_name, minor_head_code, ministry_id, sector_id, department_id, major_head_id, sub_major_head_id, budget_type) VALUES ('".$_REQUEST['minor_head_name']."', '".$_REQUEST['minor_head_code']."', '".$fetch_sub_major_head['ministry_id']."', '".$fetch_sub_major_head['sector_id']."', '".$fetch_sub_major_head['department_id']."', '".$fetch_sub_major_head['major_head_id']."', '".$fetch_sub_major_head['sub_major_head_id']."', '".$fetch_sub_major_head['budget_type']."')";
			$i_result=mysql_query($i_query);
			$latest_minor_head_id=mysql_insert_id();
			if($i_result){
				echo "<meta http-equiv='Refresh' content='0; url=add_sub_head.php?minor_headid=".$latest_minor_head_id."&level=".$_REQUEST['level']."'>";
				exit;
			} else { 
		...
		<tr>
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			<tr>
			  ...
			  <td width="41%" align="right"><a href="minor_head.php" class="back_menu">View Minor Head</a>&nbsp;&nbsp;</td>
			</tr>
			<tr>
			  <td>&nbsp; </td>
