	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
		...
		<tr>
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  <td class="backheads" align="center">Share of Budget for Children in State Social Sector Allocation </td>
			  </tr>
			<tr>
			  <td align="right">&nbsp; </td>
			</tr>
...
<tr>
			  ...
			  <td width="146" align="left"><input name="submit" type="submit" id="submit" value="Search">&nbsp;</td>
				</tr>
				...
				<tr>
				  <td height="25" colspan="7" class="txt">&nbsp;</td>
					...
					<tr>
					  ...
					  <td height="25" align="center" class="txt" colspan="4"><strong>Share of Budget for Children in Social Sector Allocation</strong></td>
					  </tr>
...
// For Excel File Generation
			...
			$_SESSION['report_header']=array("Share of Budget for Children in State Social Sector Allocation"," "); 
				$i=0;
				$_SESSION['report_values'][$i][0]=State." >> ".$fetch_state['state_name'];
				$_SESSION['report_values'][$i][1]=" ";
				$i=1;
				$_SESSION['report_values'][$i][0]=Year."  ";
				$_SESSION['report_values'][$i][1]=Share." ".of." ".Budget." "."for"." ".Children." ".in." ".State." ".Social." ".Sector." ".Allocation." ";
				$i=2;
			while($fetch_year=mysql_fetch_array($result_year)){


			$select_social_budget="select budget_be_grand_total, budget_year from haq_budget_report_union where budget_type='social_sec' and budget_status='active' and state_id='".$_REQUEST['state_id']."' and  social_sector_type='".$_REQUEST['level']."' and budget_year='".$fetch_year['year_name']."'";
			$result_social_budget=mysql_query($select_social_budget);
			$fetch_social_budget=mysql_fetch_array($result_social_budget);

			$select_be="select sum(be_grand_total) as be_total, expend_year from haq_head_expend_state where expend_type='".$_REQUEST['level']."' and expend_status='active' and state_id='".$_REQUEST['state_id']."' and expend_year='".$fetch_year['year_name']."' group by expend_year";
			$result_be=mysql_query($select_be);
			$fetch_be=mysql_fetch_array($result_be);
			$be_cro=$fetch_be['be_total']/10000;

			if($fetch_social_budget['budget_year'] !=0 && $fetch_be['expend_year'] !=0){
			if ($fetch_social_budget['budget_be_grand_total'] !=0 ) {
				$be_bfc= $be_cro/$fetch_social_budget['budget_be_grand_total']*100;
...
?>	
				...
				<tr>
				  ...
				  <td align="center" class="txt" colspan="4"><?php echo round($be_bfc, 3)."%" ; ?></td>
				  </tr>
<?php
				$i=$i+1;
}	}
...
/*			$select_soc_all="select sum(budget_be_grand_total) as soc_total from haq_budget_report_union where budget_type='social_sec' and budget_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and social_sector_type='".$_REQUEST['level']."' and state_id='".$_REQUEST['state_id']."' and budget_status='active'";
			$result_soc_all=mysql_query($select_soc_all);
			$fetch_soc_all=mysql_fetch_array($result_soc_all);

			$select_bfc_all="select sum(be_grand_total) as be_total from haq_head_expend_state where expend_type='".$_REQUEST['level']."'  and expend_year_code between '".$_REQUEST['year_from']."' and '".$_REQUEST['year_to']."' and state_id='".$_REQUEST['state_id']."' and expend_status='active'";
			$result_bfc_all=mysql_query($select_bfc_all);
			$fetch_bfc_all=mysql_fetch_array($result_bfc_all);
			$bfc_cro_total=$fetch_bfc_all['be_total']/10000;
...
///////AVERAGE CALCULATION ENDS////////////////
				...
				<tr>
				  <td height="30" colspan="7" align="center">&nbsp;<a href="export_report.php?fn=ST_share_BFC_SS_all">Download in Excel</a></td>
				  </tr>
		  </table></td>
			</tr>
			<tr>
