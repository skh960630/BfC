<?php
	...
	include ('conn.php');
		$select_other_head=mysql_query("select * from haq_other_head where other_head_id='".$_REQUEST['other_headid']."'");
	...
	if (isset($_REQUEST['submit'])){ 
		...
		if($f_query !=0){
			$error= "Sorry, Entry for same Other Head for Year ' " . $_REQUEST['bud_year'] . " ' already exist.";
		} else {
		...
		major_head_id, sub_major_head_id, minor_head_id, sub_head_id, detail_head_id, object_head_id, other_head_id, program_id, sector_id) 
		VALUES(
		...
		'".$fetch_other_head['major_head_id']."', '".$fetch_other_head['sub_major_head_id']."', '".$fetch_other_head['minor_head_id']."', '".$fetch_other_head['sub_head_id']."', '".$fetch_other_head['detail_head_id']."', '".$fetch_other_head['object_head_id']."', '".$fetch_other_head['other_head_id']."', '".$_REQUEST['program_name']."', '".$fetch_other_head['sector_id']."')";
		$result_budget=mysql_query($insert_budget);
		$update_program=mysql_query("update haq_major_head set program_id='".$_REQUEST['program_name']."' where major_head_id='".$fetch_other_head['major_head_id']."'");
			if($result_budget){
			echo "<meta http-equiv='Refresh' content='0; url=other_head.php?level=union'>";
			exit;
			}else{ 
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  ...
			  <td colspan="3" align="left"><select name="program_name">
				...
				<?
					$sel_prog="select * from haq_program where program_status='active' and sector_id='".$fetch_other_head['sector_id']."' order by program_id";
