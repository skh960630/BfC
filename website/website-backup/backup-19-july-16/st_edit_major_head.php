<?php 
	...
	include ('conn.php');
	$fetch_query=mysql_query("select * from haq_st_major_head where major_head_id='".$_REQUEST['majorheadid']."'");
	$fetch_result=mysql_fetch_array($fetch_query);
	if(isset($_REQUEST['submit'])){
		$s_query=mysql_query("select * from haq_st_major_head where  major_head_id !='".$_REQUEST['majorheadid']."' and state_id='".$fetch_result['state_id']."' and sector_id='".$fetch_result['sector_id']."' and program_id='".$fetch_result['program_id']."' and major_head_name='".$_REQUEST['major_head_name']."' and major_head_code='".$_REQUEST['major_head_code']."'");
		$f_query=mysql_num_rows($s_query);
		if($f_query !=0){
				$error= "Sorry, The Major Head " . $_REQUEST['major_head_name'] . " and Code " .$_REQUEST['major_head_code'] . " is already in use.";
		} else {
			$update_depart=mysql_query("update haq_st_major_head set major_head_name='".$_REQUEST['major_head_name']."', major_head_code='".$_REQUEST['major_head_code']."' where major_head_id='".$_REQUEST['majorheadid']. "'");
			if($update_depart){
	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
		...
		<tr>
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			<tr>
			  <td width="32%">&nbsp;</td>
			  <td width="35%" align="center" class="backheads"><strong>Update Major Head </strong></td>
			  <td width="33%" align="right"><a href="st_major_head.php" class="back_menu">View Major Head</a>&nbsp;&nbsp;</td>
			</tr>
			<tr>
			  <td>&nbsp; </td>
