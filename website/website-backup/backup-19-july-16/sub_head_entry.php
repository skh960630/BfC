<?php
	...
	include ('conn.php');
		$select_sub_head=mysql_query("select * from haq_sub_head where sub_head_id='".$_REQUEST['sub_headid']."'");
	...
	if (isset($_REQUEST['submit'])){ 
		...
		major_head_id, sub_major_head_id, minor_head_id, sub_head_id, program_id, sector_id) 
		VALUES(
		...
		'".$fetch_sub_head['major_head_id']."', '".$fetch_sub_head['sub_major_head_id']."', '".$fetch_sub_head['minor_head_id']."', '".$fetch_sub_head['sub_head_id']."', '".$_REQUEST['program_name']."', '".$fetch_sub_head['sector_id']."')";
		$result_budget=mysql_query($insert_budget);
		$update_program=mysql_query("update haq_major_head set program_id='".$_REQUEST['program_name']."' where major_head_id='".$fetch_sub_head['major_head_id']."'");
			if($result_budget){
			echo "<meta http-equiv='Refresh' content='0; url=sub_head.php?level=union'>";
			exit;
			}else{ 
		  ...
		  <table width="50%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  ...
			  <td colspan="3" align="left"><select name="program_name">
				...
				<?
					$sel_prog="select * from haq_program where program_status='active' and sector_id='".$fetch_sub_head['sector_id']."' order by program_id";
