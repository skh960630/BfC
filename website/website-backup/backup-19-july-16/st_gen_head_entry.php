<?php
	...
	include ('conn.php');
		...
		$select_gen_head=mysql_query("select * from haq_st_gen_head where gen_head_id='".$_REQUEST['gen_headid']."' $sub_query");
	...
	if (isset($_REQUEST['submit'])){ 
		...
		$s_query=mysql_query("select * from haq_head_expend_state where expend_year='".$_REQUEST['bud_year']."' and expend_status='active' and major_head_id='".$fetch_gen_head['major_head_id']."' and sub_major_head_id='".$fetch_gen_head['sub_major_head_id']."' and minor_head_id='".$fetch_gen_head['minor_head_id']."' and gen_head_id='".$fetch_gen_head['gen_head_id']."'");
		$f_query=mysql_num_rows($s_query);
		if($f_query !=0){
			$error= "Sorry, Entry for same General Head for Year ' " . $_REQUEST['bud_year'] . " ' already exist.";
		} else {
		...
		major_head_id, sub_major_head_id, minor_head_id, gen_head_id, program_id, sector_id, state_id) 
		VALUES(
		...
		'".$fetch_gen_head['major_head_id']."', '".$fetch_gen_head['sub_major_head_id']."', '".$fetch_gen_head['minor_head_id']."', '".$fetch_gen_head['gen_head_id']."', '".$_REQUEST['program_name']."', '".$fetch_gen_head['sector_id']."', '".$fetch_gen_head['state_id']."')";
		$result_budget=mysql_query($insert_budget);
		$update_program=mysql_query("update haq_st_major_head set program_id='".$_REQUEST['program_name']."' where major_head_id='".$fetch_gen_head['major_head_id']."'");
			if($result_budget){
			echo "<meta http-equiv='Refresh' content='0; url=st_gen_head.php?level=state'>";
			exit;
			}else{ 
	  ...
	  <table width="100%"  border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
		...
		<tr>
		  ...
		  <table width="56%" align="center" border="0" cellspacing="2" cellpadding="1" bgcolor="#F1F4F7">
			...
			<tr>
			  <td width="27%">&nbsp;</td>
			  <td width="33%" align="center" class="backheads">General  Head Entry </td>
			  <td width="40%" align="right"><a href="vi_gen_head_entry.php?gen_headid=<?php echo $_REQUEST['gen_headid'] ; ?>" class="back_menu">VI Schedule Entry</a>&nbsp;&nbsp;<a href="st_gen_head.php" class="back_menu">View General Head</a>&nbsp;&nbsp;</td>
			</tr>
			<tr>
			...
			<tr>
			  ...
			  <td colspan="3" align="left"><select name="program_name">
				...
				<?
					$sel_prog="select * from haq_st_program where program_status='active' and sector_id='".$fetch_gen_head['sector_id']."' and program_state_id='".$fetch_gen_head['state_id']."' order by program_id";
